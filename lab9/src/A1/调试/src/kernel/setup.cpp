...
int syscall_1(int first, int second, int third, int forth, int fifth)
{
    ...
    if (min > fifth)
        min = fifth;
    printf("systerm call 1:\nnumber-----%d, %d, %d, %d, %d-----\nmin: %d\n",
           first, second, third, forth, fifth, min);
    return min;
}
...
extern "C" void setup_kernel()
{
    ...
    int ret1;
    ret1 = asm_system_call(0, 10, 2, 3, 4);
    printf("return value: %d\n", ret1);
    ret1 = asm_system_call(1, 10, 2, 3, 4);
    printf("return value: %d\n", ret1);
    ret1 = asm_system_call(1, 123, 324, 9248, 7, 123);
    printf("return value: %d\n", ret1);
    // 创建第一个线程
