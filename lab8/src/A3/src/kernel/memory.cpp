...
/*
int MemoryManager::allocatePhysicalPages(enum AddressPoolType type, const int count,int* array,int* size,int* num);
{
    if((*num)<3)
    {
    	int start = -1;
    ...
    {
    	printf("happening the change of pages!");
    	//change
    	char* p=(char*)array[0];
    	int si=size[0];
    	for(int i=index+1;i<3;i++)
        ...
        //release
        releasePages(AddressPoolType::KERNEL, (int)p, si);
    	int start = -1;
...
int MemoryManager::allocatePages(enum AddressPoolType type, const int count,int* array,int& size,int& index)
{
...
/*
void MemoryManager::visit_array(enum AddressPoolType type,const int virtualAddress,const int count,int array[3])
{
    //look for the address
    int Is_in=0;
    int index=0;
